...
// Generates a fixed, seeded-feeling star field once — not re-randomized on every render
...
const StarField = () => {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden rounded-2xl">
      ...
      {/* A few larger "shooting" stars for extra depth, subtler frequency */}
      {[0, 1].map((i) => (
        <motion.span
          key={`bright-${i}`}
          className="absolute h-[3px] w-[3px] rounded-full bg-[#818CF8]"
          style={{ left: `${20 + i * 45}%`, top: `${15 + i * 30}%` }}
...
const AIAssistant = () => {
  ...
  const handleRobotMove = (e: React.MouseEvent<HTMLDivElement>) => {
  ...
  useEffect(() => {
    const timers = chatBubbles.map((bubble, i) =>
      window.setTimeout(() => setVisibleStep(i + 1), bubble.delay * 1000)
    );
  ...
  return (
    <section className="mx-auto max-w-[1280px] px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
      <div className="relative grid grid-cols-1 gap-8 overflow-hidden rounded-2xl bg-[#0B1020] p-6 sm:p-10 lg:grid-cols-2 lg:items-center lg:p-14">
        ...
        {/* Star field — sits behind everything in the whole card */}
        <StarField />
        ...
        >
          ...
          <div className="relative z-10">
            ...
            {/* AI Robot — 3D tilt + orbiting particles */}
        ...
        >
          ...
          <AnimatePresence mode="wait">
            {visibleStep === 1 && (
              ...
              >
                <img src={assets.aiassistant} alt="" className="h-7 w-7 shrink-0 object-contain" />
            ...
            {visibleStep >= 2 && (
              ...
              >
                <img src={assets.aiassistant} alt="AI Assistant" className="mt-1 h-7 w-7 shrink-0 object-contain" />
            ...
            {visibleStep >= 3 && (
              ...
              >
                {[1, 2, 3].map((item) => (
                  ...
                  >
                    <img
                      src={item === 1 ? assets.macbook : item === 2 ? assets.laptop : assets.macbook}
          ...
          <AnimatePresence>
            {visibleStep >= 4 && (
              <motion.button
                ...
                transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
                whileHover={{ backgroundColor: "#F8F9FC", scale: 1.01 }}
                whileTap={{ scale: 0.98 }}
